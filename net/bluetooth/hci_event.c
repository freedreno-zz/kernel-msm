...
static inline void hci_conn_complete_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	BT_ERR("HCI Conn. lookup for BDADDR(%s) of type: %d",
		batostr(&ev->bdaddr), ev->link_type);
	conn = hci_conn_hash_lookup_ba(hdev, ev->link_type, &ev->bdaddr);
	if (!conn) {
		BT_ERR("No HCI Conn. exits for BDADDR(%s) of type: %d",
			batostr(&ev->bdaddr), ev->link_type);
		...
		BT_ERR("HCI Conn. exists for BDADDR(%s) of type: %d",
...
static inline void hci_conn_request_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	BT_ERR("%s bdaddr: %s link-type: 0x%x", hdev->name,
					batostr(&ev->bdaddr), ev->link_type);
	...
	if ((mask & HCI_LM_ACCEPT) &&
		...
		BT_ERR("Check ACL connection exists for BDADDR(%s) of type: %d",
			batostr(&ev->bdaddr), ev->link_type);
		conn = hci_conn_hash_lookup_ba(hdev, ev->link_type, &ev->bdaddr);
		if (!conn) {
			/* pkt_type not yet used for incoming connections */
			BT_ERR("NO ACL conn. exists for BDADDR: %s of type: %d",
				batostr(&ev->bdaddr), ev->link_type);
			...
			BT_ERR("ACL already exists for BDADDR: %s of type: %d",
				batostr(&ev->bdaddr), ev->link_type);
...
static inline void hci_num_comp_pkts_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	for (i = 0, ptr = (__le16 *) skb->data; i < ev->num_hndl; i++) {
		...
		if (hdev->dev_type == HCI_BREDR) {
			BT_DBG("%s: Device type: BR/EDR", __func__);
			conn = hci_conn_hash_lookup_handle(hdev, handle);
		}
		else {
			BT_DBG("%s: Device type: AMP", __func__);
...
static inline void hci_link_key_request_evt(struct hci_dev *hdev, struct sk_buff *skb)
{
	...
	if (!key) {
		BT_ERR("%s link key not found for %s", hdev->name,
							batostr(&ev->bdaddr));
		goto not_found;
	}

	BT_ERR("%s found key type: %u for: %s", hdev->name, key->key_type,
							batostr(&ev->bdaddr));
