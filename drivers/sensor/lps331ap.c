...
struct sensor_regulator lps331_vreg[] = {
	{NULL, "vdd2v9", 2400000, 3300000},
	{NULL, "vdd1v8", 1800000, 1900000},
};

static int lps331_config_regulator(struct lps331ap_prs_data *acc, bool on)
{
	int rc = 0, i;
	int num_reg = sizeof(lps331_vreg) / sizeof(struct sensor_regulator);
	...
	if (on) {
		for (i = 0; i < num_reg; i++) {
			lps331_vreg[i].vreg =
				...
				lps331_vreg[i].name);
			if (IS_ERR(lps331_vreg[i].vreg)) {
				rc = PTR_ERR(lps331_vreg[i].vreg);
			...
			if (regulator_count_voltages(
				lps331_vreg[i].vreg) > 0) {
				rc = regulator_set_voltage(
					lps331_vreg[i].vreg,
					lps331_vreg[i].min_uV,
					lps331_vreg[i].max_uV);
				if (rc) {
					pr_err("%s: set voltage failed rc=%d\n",
					__func__, rc);
					regulator_put(lps331_vreg[i].vreg);
					goto error_vdd;
				}
			}

			rc = regulator_enable(lps331_vreg[i].vreg);
			if (rc) {
...
error_vdd:
	while (--i >= 0) {
		if (regulator_count_voltages(lps331_vreg[i].vreg) > 0) {
			regulator_set_voltage(lps331_vreg[i].vreg, 0,
						lps331_vreg[i].max_uV);
		}
		regulator_disable(lps331_vreg[i].vreg);
		regulator_put(lps331_vreg[i].vreg);
	}
	return rc;
}
...
#ifdef DEBUG
	dev_dbg(&prs->client->dev, " tH = 0x%02x, tL = 0x%02x,"
...
#ifdef DEBUG
	dev_dbg( &prs->client->dev , "Pressure %d mbar, temp = %d C\n",out->press, out->temperature);
#endif
...
static irqreturn_t lps331ap_prs_interrupt_thread(int irq\
	, void *lps331ap_prs_data_p)
{
	struct lps331ap_prs_data *prs = lps331ap_prs_data_p;
					...
					const struct i2c_device_id *id)
{
	...
	int error;
	int tempvalue;
	const struct lps331ap_platform_data *pdata = client->dev.platform_data;
	...
	lps331_config_regulator(prs,true);
	msleep(200);
...
/*
	prs->resume_state[LPS331AP_RES_CTRL_REG1] =
				(LPS331AP_PRS_PM_NORMAL | LPS331AP_PRS_ODR_1_1 |
					LPS331AP_PRS_BDU_ON));
	prs->resume_state[LPS331AP_RES_CTRL_REG2] = 0x;
	prs->resume_state[LPS331AP_RES_CTRL_REG3] = 0x24;
	...
	prs->resume_state[LPS331AP_RES_CTRL_REG3] = 0x24; 
	/*Latch interrupt request into INT_SOURCE and enable data ready interrupt */
	prs->resume_state[LPS331AP_RES_INT_CFG_REG] = 0x07;  
	...
	if (prs->client->irq > 0) { /* interrupt */
		//memcpy(&data->ctrl_regs, &default_ctrl_regs_fifo,
		//	sizeof(default_ctrl_regs_fifo));
		prs->interruptible = true;
		//data->entries = 1;
		err = request_threaded_irq(prs->client->irq, NULL,
			lps331ap_prs_interrupt_thread\
