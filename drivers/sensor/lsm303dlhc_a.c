...
struct sensor_regulator lsm303dlhc_acc_vreg[] = {
	{NULL, "vdd2v9", 2400000, 3300000},
	{NULL, "vdd1v8", 1800000, 1900000},
};

static int lsm303dlhc_acc_config_regulator(struct lsm303dlhc_acc_data *acc, bool on)
{
	int rc = 0, i;
	int num_reg = sizeof(lsm303dlhc_acc_vreg) / sizeof(struct sensor_regulator);

	if (on) {
		for (i = 0; i < num_reg; i++) {
			lsm303dlhc_acc_vreg[i].vreg =
				regulator_get(&acc->client->dev,
				lsm303dlhc_acc_vreg[i].name);
			if (IS_ERR(lsm303dlhc_acc_vreg[i].vreg)) {
				rc = PTR_ERR(lsm303dlhc_acc_vreg[i].vreg);
			...
			if (regulator_count_voltages(
				lsm303dlhc_acc_vreg[i].vreg) > 0) {
				rc = regulator_set_voltage(
					lsm303dlhc_acc_vreg[i].vreg,
					lsm303dlhc_acc_vreg[i].min_uV,
					lsm303dlhc_acc_vreg[i].max_uV);
				if (rc) {
					pr_err("%s: set voltage failed rc=%d\n",
					__func__, rc);
					regulator_put(lsm303dlhc_acc_vreg[i].vreg);
					goto error_vdd;
				}
			}

			rc = regulator_enable(lsm303dlhc_acc_vreg[i].vreg);
			if (rc) {
...
error_vdd:
	while (--i >= 0) {
		if (regulator_count_voltages(lsm303dlhc_acc_vreg[i].vreg) > 0) {
			regulator_set_voltage(lsm303dlhc_acc_vreg[i].vreg, 0,
						lsm303dlhc_acc_vreg[i].max_uV);
		}
		regulator_disable(lsm303dlhc_acc_vreg[i].vreg);
		regulator_put(lsm303dlhc_acc_vreg[i].vreg);
	}
	return rc;
}
...
static ssize_t attr_set_value(struct device *dev,
				     struct device_attribute *attr,
				     const char *buf, size_t size)
{
	return size;
...
{
	write_reg(dev, buf, INT_CFG1, NO_MASK, RES_INT_CFG1);
	return size;
}
...
static ssize_t attr_set_duration1(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, INT_DUR1, INT1_DURATION_MASK, RES_INT_DUR1);
	return size;
}
...
static ssize_t attr_set_thresh1(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, INT_THS1, INT1_THRESHOLD_MASK, RES_INT_THS1);
	return size;
}
...
static ssize_t attr_set_click_cfg(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, TT_CFG, TAP_CFG_MASK, RES_TT_CFG);
	return size;
}
...
static ssize_t attr_set_click_ths(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, TT_THS, TAP_THS_MASK, RES_TT_THS);
	return size;
}
...
static ssize_t attr_set_click_tlim(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, TT_LIM, TAP_TLIM_MASK, RES_TT_LIM);
	return size;
}
...
static ssize_t attr_set_click_tlat(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, TT_TLAT, TAP_TLAT_MASK, RES_TT_TLAT);
	return size;
}
...
static ssize_t attr_set_click_tw(struct device *dev,
		struct device_attribute *attr,	const char *buf, size_t size)
{
	write_reg(dev, buf, TT_TLAT, TAP_TW_MASK, RES_TT_TLAT);
	return size;
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(get_value, 0664, attr_get_value, attr_set_value),
		...
		const struct i2c_device_id *id)
{
	...
	i2c_set_clientdata(client, acc);
        lsm303dlhc_acc_config_regulator(acc, true);
	...
	err = lsm303dlhc_acc_update_g_range(acc, acc->pdata->g_range);
	if (err < 0) {
