					  ...
					  struct device *dev)
{
	...
	wcn->tx_enable_state = qcom_smem_state_get(dev, "tx-enable", &wcn->tx_enable_state_bit);
	if (IS_ERR(wcn->tx_enable_state)) {
		wcn36xx_err("failed to get tx-enable state\n");
		return -ENOENT;
	}

	wcn->tx_rings_empty_state = qcom_smem_state_get(dev, "tx-rings-empty", &wcn->tx_rings_empty_state_bit);
...
{
	return qcom_smem_state_update_bits(wcn->tx_enable_state, clear_mask | set_mask, set_mask);
}
