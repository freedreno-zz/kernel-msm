...
int mdp4_dtv_pipe_commit(int cndx, int wait, u32 *release_busy)
{
	...
	if (vctrl->blt_free) {
		vctrl->blt_free--;
		if (vctrl->blt_free == 0) {
			pr_info("%s: free writeback buffer\n", __func__);
...
void mdp4_dtv_vsync_ctrl(struct fb_info *info, int enable)
{
	...
	if (!external_common_state->hpd_state) {
		vctrl->vsync_event++;
		wake_up_interruptible_all(&vctrl->vsync_queue);
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ret <= 0) {
		if (vctrl->vsync_timout_cnt)
			pr_err("timeout for VSYNC ret=%d\n", ret);

		if (vctrl->vsync_irq_enabled)
			vctrl->vsync_timout_cnt++;
...
void mdp4_dtv_mute(u32 enable)
{
	...
	pr_info("%s en=%d", __func__, enable);
}
...
static void mdp4_dtv_do_blt(struct msm_fb_data_type *mfd, int enable)
{
	...
	vctrl->blt_change = 0;
	tg_enabled = inpdw(MDP_BASE + DTV_BASE) & 0x01;
	if (tg_enabled) {
		mdp4_dtv_wait4vsync(cndx);
		MDP_OUTP(MDP_BASE + DTV_BASE, 0);
...
static int mdp4_dtv_csc_init(struct msm_fb_data_type *mfd) {
	...
	pr_info("%s use_csc_limited: %d quant: %d ",
