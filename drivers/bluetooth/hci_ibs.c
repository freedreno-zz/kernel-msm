...
static inline int ibs_check_data_len(struct hci_uart *hu,
	...
	while (count) {
		if (ibs->rx_count) {
			...
			case HCI_IBS_W4_EVENT_HDR:
				...
				ibs_check_data_len(hu, ibs, eh->plen);
				continue;
			...
			case HCI_IBS_W4_ACL_HDR:
				...
				ibs_check_data_len(hu, ibs, dlen);
				continue;
			...
			case HCI_IBS_W4_SCO_HDR:
				...
				ibs_check_data_len(hu, ibs, sh->dlen);
				continue;
			}
		}
