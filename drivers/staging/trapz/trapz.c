...
#ifdef CONFIG_TRAPZ_TRIGGER
/* The trigger implementation is admittedly not performant for very many triggers.  We have
   weak requirements around triggers, and thus a simple, small linked list is our current
...
static struct _trapz_device_info {
	...
	atomic_t enabled;                      // indicates if device is enabled
	atomic_t blocked;
	int pending_buffsize_change;           // indicates if there is a pending buffer resize
...
static struct trapz_data {
	...
	u8 counter;                            // the rolling data counter (1 thru 255)
...
static void get_loglevel(trapz_filters * filter_buffer);
...
struct file_operations trapz_fops = {
   .llseek = trapz_llseek,
   .read = trapz_read,
   .write = trapz_write,
   .open = trapz_open,
 ...
 */
SYSCALL_DEFINE6(trapz, unsigned int, ctrl, unsigned int, extra1, unsigned int, extra2, unsigned int, extra3, unsigned int, extra4, struct trapz_info __user *, ti)
{
...
static inline trapz_entry_t* get_entry(void) {
	...
	if (g_data.count >= g_data.bufferSize) {
		// We've wrapped the circular buffer.  Move the tail accordingly.
		g_data.pTail++;
 ...
 */
long systrapz(unsigned int ctrl, unsigned int extra1, unsigned int extra2, unsigned int extra3, unsigned int extra4, struct trapz_info __user *ti)
{
	unsigned long flags;
	...
	if (trapz_check_loglevel(level, cat_id, comp_id)) {
		...
		getnstimeofday(&ts);
		filtered = 0;
		//Determine if we need an additional record to hold the extras
		if (extra3 != 0 || extra4 != 0 || extra1 > 0xff || extra2 > 0xff) {
			// Need extra record
			extra_count = 1;
		}
		spin_lock_irqsave(&trapz_device_info.lock, flags);
		{
			...
			process_trigger(ctrl, &trigger_event, &trigger_head, &g_trigger_count, &ts);
#endif
	...
	pEntry1->comp_trace_id[1] = ((comp_id & 0x0f) << 4) | ((trace_id & 0xf00) >> 8);
	...
	if (trigger_event.trigger.start_trace_point != 0) {
		send_trigger_uevent(&trigger_event, &(trapz_device_info.trapz_device.this_device->kobj));
	}
#endif
	...
	for (;;) {
		...
		if (*offset >= g_data.total) {
			if(filp->f_flags & O_NONBLOCK)
				...
				if (wait_event_interruptible(trapz_device_info.wq, atomic_read(&trapz_device_info.blocked) == 0)
				    == -ERESTARTSYS)
				{
					return 0;
				}
	...
	if (atomic_read(&trapz_device_info.enabled) != 0 && component_id >= 0) {
		if (level >= TRAPZ_LOG_VERBOSE && level <= TRAPZ_LOG_OFF) {
			...
			if (component_id <= TRAPZ_MAX_COMP_ID) {
				...
				bufflevel = ((buffer[byteIdx] & (TRAPZ_LOG_OFF << bitIdx)) >> bitIdx);
				if (bufflevel <= level) {
					rc = 1;
				}
			}
...
static void set_comp_loglevel(char *buffer, const int level, const int component_id) {
	...
	if (level >= TRAPZ_LOG_VERBOSE && level <= TRAPZ_LOG_OFF &&
		...
		buffer[byteIdx] = (buffer[byteIdx] & ((TRAPZ_LOG_OFF << bitIdx) ^ 0xFF)) | bufflevel;
	}
}
...
static void set_loglevel(const int level, const int cat_id, const int component_id) {
	...
	if (component_id >= 0) {
		...
		if (cat_id >= 0) {
			...
			if (cat_id == TRAPZ_CAT_APPS) {
				memset(g_data.app_comp_loglevels, buffval, TRAPZ_LOG_LEVEL_BUFF_SIZE);
			} else {
				memset(g_data.os_comp_loglevels, buffval, TRAPZ_LOG_LEVEL_BUFF_SIZE);
			}
		} else {
...
static const char * level_to_string(int level)
{
	...
	levStr = (level == TRAPZ_LOG_VERBOSE) ? "verbose" : 
					(level == TRAPZ_LOG_DEBUG) ? "debug" :
							(level == TRAPZ_LOG_INFO) ? "info" : "off";
									
	return levStr;										
}

/* Get the log level filters currently in effect. */
static void get_loglevel(trapz_filters * filter_buffer)
{
	int cat_idx;
	char *buffer;
	...
	int os_count = 0;
	int apps_count = 0;
	int all_os_level = TRAPZ_LOG_OFF;
	int all_apps_level = TRAPZ_LOG_OFF;
	int level;
	int entry_max = 0;
	int i;
	...
	//
	// Notes: 
	//   - TRAPZ_LOG_LEVEL_BUFF_SIZE == 1024
	//   - TRAPZ_MAX_COMP_ID == 4095 which means that 4096 id's can fit in 
	//     the os_comp_loglevels or app_comp_loglevels buffers.
	//
	...
	if (atomic_read(&trapz_device_info.enabled) == 0)
	{
		...
		filter_buffer->os_level = TRAPZ_LOG_OFF;
		filter_buffer->apps_level = TRAPZ_LOG_OFF;
	...
	//
	// Calculate the number of array entries in 'filter_buffer->entry[]'.
	//
	entry_max = (filter_buffer->size - (sizeof(trapz_filters) - sizeof(filter_buffer->entry))) / sizeof(filter_buffer->entry[0]);
	
	//
	// Increment for each entry added. Add no more than 'entry_max' entries.
	//
	filter_buffer->count = 0;  


	//
	// Look at the OS/Platform and Apps filter buffers. Index 0 = OS/Platform, Index 1 = APPS.
	//
	for (cat_idx = 0; cat_idx < 2; cat_idx++)
	{
		// Pick the buffer to look at.
		if (cat_idx == 0)
			buffer = g_data.os_comp_loglevels;   // OS/Platform
		else
			buffer = g_data.app_comp_loglevels;  // APPS


		//
		// Check each entry in the buffer.
		//
		// Note: If iterating through the buffer, get the component ID this way -->  component_id = (index << 2);
		//
		for (component_id = 0; component_id <= TRAPZ_MAX_COMP_ID; component_id++)
		{
			byteIdx = component_id >> 2;
			bitIdx = (component_id % 4) << 1;
			if (byteIdx >= TRAPZ_LOG_LEVEL_BUFF_SIZE)
			{
				printk(KERN_ERR "trapz: get_loglevel(): index %d greater than buffer size = %d\n", byteIdx, TRAPZ_LOG_LEVEL_BUFF_SIZE);
				continue;
			}

			level = ((buffer[byteIdx] & (TRAPZ_LOG_OFF << bitIdx)) >> bitIdx);
			
			//
			// Add the level to the callers array if the level is set to anything other than OFF.
			//
			if (level != TRAPZ_LOG_OFF)
			{
				if (cat_idx == 0) // OS/Platform
				{
					++os_count;
					
					// Capture the level that ALL os and platforms are set to.
					if (all_os_level == TRAPZ_LOG_OFF)
						all_os_level = level;    
				}
				else              // APPS
				{
					...
					// Capture the level that ALL apps are set to.
					if (all_apps_level == TRAPZ_LOG_OFF)
						all_apps_level = level;  
				}
					
				// If there's room in the callers buffer, add it...
				if (filter_buffer->count < entry_max)
				{
					filter_buffer->entry[filter_buffer->count] = TRAPZ_FILTER_SET_ENTRY(level, component_id);
					++filter_buffer->count;
				}
			}
		}
	...
	printk(KERN_INFO "trapz: os/platforms enabled = %d (level=%s), apps enabled = %d (level=%s)\n", 
						os_count,   level_to_string(all_os_level), 
						apps_count, level_to_string(all_apps_level));

	if ((apps_count == 0) && (os_count == 0))
	{
		//
		// Case: All tracing is OFF.
		//
		// filters affect all apps and os/platform components.
		//
		filter_buffer->count = 0;
		filter_buffer->os_level = TRAPZ_LOG_OFF;
		filter_buffer->apps_level = TRAPZ_LOG_OFF;

		printk(KERN_INFO "trapz: OS level=OFF, APPS level=OFF\n");
	}
	else if ((apps_count >= TRAPZ_MAX_COMP_ID) && (os_count >= TRAPZ_MAX_COMP_ID))
	{
		//
		// Case: All tracing is ON.  
		//
		// filters affect all apps and os/platform components.
		//
		filter_buffer->count = 0;
		filter_buffer->os_level = all_os_level;
		filter_buffer->apps_level = all_apps_level;
		
		printk(KERN_INFO "trapz: OS level=%s, APPS level=%s\n", level_to_string(all_os_level), level_to_string(all_apps_level));
	}
	else if ((apps_count >= TRAPZ_MAX_COMP_ID) && (os_count == 0))
	{
		//
		// Case: All tracing is ON for the APPS category.  
		//
		// filters affect all apps components.
		//
		filter_buffer->count = 0;
		filter_buffer->os_level = TRAPZ_LOG_OFF;
		filter_buffer->apps_level = all_apps_level;

		printk(KERN_INFO "trapz: OS level=OFF, APPS level=%s\n", level_to_string(all_apps_level));
	}
	else if ((apps_count == 0) && (os_count >= TRAPZ_MAX_COMP_ID))
	{
		//
		// Case: All tracing is ON for the OS/PLATFORM category.  
		//
		// filters affect all os components.
		//
		filter_buffer->count = 0;
		filter_buffer->os_level = all_os_level;
		filter_buffer->apps_level = TRAPZ_LOG_OFF;

		printk(KERN_INFO "trapz: OS level=%s, APPS level=OFF\n", level_to_string(all_os_level));
	}
	else
	{
		//
		// Case: Various app/os/platform components are set to various levels.
		//
		// filter_buffer->entry[] and filter_buffer->count have been set above in the loop.
		//

		filter_buffer->os_level = -1;
		filter_buffer->apps_level = -1;
#if 1		
		printk(KERN_INFO "trapz: OS & APPS, count = %d (total %d)\n", filter_buffer->count, apps_count + os_count);

		for (i = 0; i < filter_buffer->count; i++)
		{
			level = TRAPZ_FILTER_GET_LEVEL(filter_buffer->entry[i]);
			component_id = TRAPZ_FILTER_GET_COMPONENT_ID(filter_buffer->entry[i]);
			
			printk(KERN_INFO "    component %d = %s\n", component_id, level_to_string(level));
		}
#endif
	}
}
...
static int allocate_mem(int init_flags) {
	...
	if (((init_flags & 1) != 0) && g_data.bufferSize > 0) {
		...
		g_data.pLimit = g_data.pHead = g_data.pTail = g_data.pBuffer =
			(trapz_entry_t *) kmalloc(sizeof(trapz_entry_t) * g_data.bufferSize, GFP_KERNEL);
	...
	if (ok && ((init_flags & 2) != 0)) {
		...
		g_data.os_comp_loglevels = kmalloc(TRAPZ_LOG_LEVEL_BUFF_SIZE, GFP_KERNEL);
		g_data.app_comp_loglevels = kmalloc(TRAPZ_LOG_LEVEL_BUFF_SIZE, GFP_KERNEL);
...
static int enable_driver(int enable) {
	...
	if (enable) {
		// Enabling device.  Check to see if there is a pending buffer size change.
		// If so, we need to re-allocate memory.
		if (trapz_device_info.pending_buffsize_change) {
			...
			if (rc == 0) {
				...
				printk(KERN_ERR "trapz: could not enable trapz - out of memory\n");
				g_data.bufferSize = 0;
			}
		} else {
...
static int set_buff_size(int new_buff_size) {
	...
	if (atomic_read(&trapz_device_info.enabled) == 0) {
		if (new_buff_size > 0 && new_buff_size <= 1000000) {
			...
			printk(KERN_INFO "trapz: attempted to set attribute buffer size to %d "
					"value must be between 0 an 1000000\n", new_buff_size);
		}
	} else {
		printk(KERN_INFO "trapz: attempted to set buffer size when driver is active!\n");
		rc = -EINVAL;
	}
...
static long trapz_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
...
#endif
	trapz_filters * filter_buffer_user;
	trapz_filters * filter_buffer_kernel;
	char stack_buffer[100];  // about 50 entries
	...
	switch (cmd) {
		...
		case TRAPZ_GET_VERSION:
			...
			rc = 0;
			if (copy_to_user((void __user *)arg, TRAPZ_VERSION, strlen(TRAPZ_VERSION)+1)) {
				rc = -EFAULT;
			}
			break;
		case TRAPZ_SET_LOG_LEVEL:
			rc = 0;
			level = (arg & (TRAPZ_LEVEL_MASK << (TRAPZ_CAT_ID_SIZE + TRAPZ_COMP_ID_SIZE + 2)))
				>> (TRAPZ_CAT_ID_SIZE + TRAPZ_COMP_ID_SIZE + 2);
			cat_id = -1;
			component_id = -1;
			if (arg & (1 << (TRAPZ_CAT_ID_SIZE + TRAPZ_COMP_ID_SIZE + 1))) {
				cat_id = (arg & (TRAPZ_CAT_ID_MASK << TRAPZ_COMP_ID_SIZE))
					>> TRAPZ_COMP_ID_SIZE;
			}
		...
		case TRAPZ_ADD_TRIGGER:
			if (copy_from_user(&trigger, (void __user *) arg, sizeof(trigger))) {
				...
				spin_lock_irqsave(&trapz_device_info.lock, flags);
				{
					rc = add_trigger(&trigger, &trigger_head, &g_trigger_count);
				}
				spin_unlock_irqrestore(&trapz_device_info.lock, flags);
			}
			break;
		case TRAPZ_DEL_TRIGGER:
			if (copy_from_user(&trigger, (void __user *) arg, sizeof(trigger))) {
				...
				spin_lock_irqsave(&trapz_device_info.lock, flags);
				{
					rc = delete_trigger(&trigger, &trigger_head, &g_trigger_count);
				}
				spin_unlock_irqrestore(&trapz_device_info.lock, flags);
			}
			break;
		...
		case TRAPZ_CNT_TRIGGERS:
			...
			break;
#endif
		case TRAPZ_GET_LOG_LEVEL:
			rc = 0;
			filter_buffer_user = (trapz_filters *)arg;  // need the 'size'
			
			if (filter_buffer_user->size > sizeof(stack_buffer))
			{
				// Need to allocate kernel space.
				filter_buffer_kernel = (trapz_filters *)kmalloc(filter_buffer_user->size, GFP_KERNEL);
				filter_buffer_kernel->size = filter_buffer_user->size;
			}
			else
			{
				// The stack based buffer is large enough.
				filter_buffer_kernel = (trapz_filters *)&stack_buffer[0];
				filter_buffer_kernel->size = filter_buffer_user->size;  // use the callers size... its <= to what the stack buffer size is.
			}

			get_loglevel(filter_buffer_kernel);
			
			if (copy_to_user((void __user *)filter_buffer_user, filter_buffer_kernel, filter_buffer_user->size) != 0)
				rc = -EFAULT;

			if (filter_buffer_kernel != (trapz_filters *)&stack_buffer[0])
				kfree(filter_buffer_kernel);
...
static ssize_t attr_show(struct device *dev, struct device_attribute *attr,
	...
	if (strcmp(attr->attr.name, "version") == 0) {
		return sprintf(buf, "%s", TRAPZ_VERSION);
	} else if (strcmp(attr->attr.name, "enabled") == 0) {
		return sprintf(buf, "%d", atomic_read(&trapz_device_info.enabled));
	} else if (strcmp(attr->attr.name, "buff_size") == 0) {
		return sprintf(buf, "%d", g_data.bufferSize);
	} else if (strcmp(attr->attr.name, "count") == 0) {
		return sprintf(buf, "%d", g_data.count);
	} else if (strcmp(attr->attr.name, "total") == 0) {
		return sprintf(buf, "%d", g_data.total);
	}
...
static ssize_t attr_store(struct device *dev, struct device_attribute *attr,
	...
	if (strcmp(attr->attr.name, "enabled") == 0) {
		...
		set_buff_size(simple_strtoul(buf, NULL, 0));
...
static int __init trapz_sysfs_init(void) {
	...
	rc = sysfs_create_file(&(trapz_device_info.trapz_device.this_device->kobj),
...
static int __init device_init(void) {
	...
	trapz_device_info.trapz_device.fops = &trapz_fops;
