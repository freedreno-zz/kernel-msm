...
int add_trigger(const trapz_trigger_t *trigger, struct list_head *head, int *trigger_count) {
	int rc = 0;
...
int delete_trigger(const trapz_trigger_t *trigger, struct list_head *head, int *trigger_count) {
	int rc = -EINVAL;
...
void send_trigger_uevent(const trapz_trigger_event_t *trigger_event, struct kobject *kobj) {
	char *envp[8];
	int i = 0;
...
void process_trigger(const int ctrl, trapz_trigger_event_t *trigger_event, struct list_head *head,
		int *trigger_count, struct timespec *ts) {
	int trigger_rc;
	struct trigger_list *found_trig;

	trigger_rc = find_trace_pt(ctrl & TRAPZ_TRIGGER_MASK, &found_trig, head);
	if (trigger_rc == 1) {
		...
		if (found_trig->trigger.single_shot) {
			if (delete_trigger(&found_trig->trigger, head, trigger_count) != 0) {
