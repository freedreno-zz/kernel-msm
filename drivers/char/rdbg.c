	...
	uint32_t num_blocks)
{
	...
	block_map->map = vzalloc(num_blocks);
	if (!block_map->map)
		return SMQ_ENOMEMORY;
...
static int smq_alloc_send(struct smq *smq, const uint8_t *pcb, int nsize)
{
	 ...
	 (SM_VERSION == smq->hdr->consumer_version)) {
		if (smq->out->s.index_check_queue_for_reset ==
			...
			while (smq->out->s.index_free_read !=
				...
				struct smq_node *pNode;

				pNode = (struct smq_node *)(
					smq->in->free +
					smq->out->s.index_free_read);
				if (pNode->index_block >= smq->num_blocks) {
					err = SMQ_EBADPARM;
					goto bail;
				}
	...
	num_blocks = (nsize + SM_BLOCKSIZE - 1) / SM_BLOCKSIZE;
...
static ssize_t rdbg_read(struct file *filp, char *buf, size_t size,
	loff_t *offset)
{
	int err = 0;
	...
	if (!rdbgdata->device_opened) {
		dev_err(rdbgdata->device, "%s: Device not opened yet",
			__func__);
		err = -ENODEV;
		goto bail;
	}
...
static ssize_t rdbg_write(struct file *filp, const char *buf, size_t size,
	loff_t *offset)
{
	int err = 0;
	struct rdbg_data *rdbgdata = filp->private_data;
...
static int __init rdbg_init(void)
{
	...
	char *node_name = vzalloc(max_len);
	...
	if (rdbgdevice->num_devices < 1 || rdbgdevice->num_devices > 65536) {
		pr_err("rgdb: invalid num_devices");
		err = -EDOM;
		goto name_bail;
	}

	rdbgdevice->rdbg_data = vzalloc(sizeof(struct rdbg_data) *
	...
	for (minor = 0; minor < rdbgdevice->num_devices; minor++) {
		...
		rdbgdevice->rdbg_data[minor].gpio_initialized = 1;
...
static void __exit rdbg_exit(void)
{
	...
	for (minor = 0; minor < rdbgdevice->num_devices; minor++) {
		if (rdbgdevice->rdbg_data[minor].gpio_initialized) {
			device_destroy(rdbgdevice->class,
				MKDEV(MAJOR(rdbgdevice->dev_no), minor));
			if (rdbgdevice->rdbg_data[minor].device_opened)
				free_irq(rdbgdevice->rdbg_data[minor].in.
					irq_base_id, (void *)
					&rdbgdevice->rdbg_data[minor]);
		}
	}
	class_destroy(rdbgdevice->class);
	cdev_del(&rdbgdevice->cdev);
