...
/* When power is turned off to this panel a minimum off time of
   500ms has to be observed before powering back on as there's no
   external reset pin. Keep track of earliest wakeup time and delay
   subsequent prepare call accordingly */
...
static int wuxga_nt_panel_prepare(struct drm_panel *panel)
{
	...
	/* A minimum delay of 250ms is required after power-up until
