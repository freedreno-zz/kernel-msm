...
static void bs_init(struct mdp4_dsi_encoder *mdp4_dsi_encoder)
{
	...
	if (!dsi_pdata) {
		dev_err(dev->dev, "could not find dsi pdata\n");
		return;
	}
...
static void mdp4_dsi_encoder_dpms(struct drm_encoder *encoder, int mode)
{
	...
	if (enabled) {
		 ...
		 */
		mdp4_irq_wait(mdp4_kms, MDP4_IRQ_PRIMARY_VSYNC);
		...
		struct drm_display_mode *adjusted_mode)
{
	struct mdp4_dsi_encoder *mdp4_dsi_encoder = to_mdp4_dsi_encoder(encoder);
	struct mdp4_kms *mdp4_kms = get_kms(encoder);
	uint32_t hsync_skew, vsync_period, vsync_len, ctrl_pol;
	...
	hsync_skew = 0;  /* get this from panel? */
	...
	mdp4_write(mdp4_kms, REG_MDP4_DSI_HSYNC_SKEW, hsync_skew);
...
static void mdp4_dsi_encoder_commit(struct drm_encoder *encoder)
{
	...
	mdp4_crtc_set_intf(encoder->crtc, INTF_DSI_VIDEO);
...
#if 0
	struct mdp4_dsi_encoder *mdp4_dsi_encoder = to_mdp4_dsi_encoder(encoder);
	return clk_round_rate(mdp4_dsi_encoder->src_clk, rate);
#else
	return rate; // ???
#endif
}
