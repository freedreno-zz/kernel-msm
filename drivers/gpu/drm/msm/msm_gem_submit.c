...
static int submit_fence_sync(struct msm_gem_submit *submit, bool explicit_sync)
{
	int i, ret = 0;
	...
	for (i = 0; i < submit->nr_bos; i++) {
		...
		ret = msm_gem_sync_object(&msm_obj->base, submit->gpu->fctx,
				write, explicit_sync);
		if (ret)
			break;
	}
		...
		struct drm_file *file)
{
	...
	/* wait on in-fence before acquiring struct_mutex, otherwise we can
	 * block retire_worker which might be the thing signaling the fence
	 * (or the thing signaling the thing that signals the fence, and so
	 * on)
	 */
	...
	ret = submit_fence_sync(submit, !!(args->flags & MSM_SUBMIT_FENCE_FD_IN));
	if (ret)
		goto out;
