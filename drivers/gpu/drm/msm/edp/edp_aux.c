...
static int edp_msg_fifo_tx(struct edp_aux *aux, struct drm_dp_aux_msg *msg)
{
	...
	for (i = 0; i < len; i++) {
		...
		reg <<= 8;
		reg &= 0x00ff00; /* index = 0, write */
		if (i == 0)
			reg |= BIT(31);  /* INDEX_WRITE */
		edp_write(aux->base + EDP_AUX_DATA, reg);
	...
	reg |= BIT(9); /* GO */
	edp_write(aux->base + EDP_AUX_TRANS_CTRL, reg);
...
static int edp_msg_fifo_rx(struct edp_aux *aux, struct drm_dp_aux_msg *msg)
{
	...
	data |= BIT(0);		/* read */
	edp_write(aux->base + EDP_AUX_DATA, data);
	...
	/* discard first byte */
	data = edp_read(aux->base + EDP_AUX_DATA);
	for (i = 0; i < len; i++) {
...
ssize_t edp_aux_transfer(struct drm_dp_aux *drm_aux, struct drm_dp_aux_msg *msg)
{
	...
	if (ret <= 0) {
		 ...
		 */
		edp_write(aux->base + EDP_AUX_TRANS_CTRL, 0);
...
void msm_edp_aux_ctrl(void *edp_aux, int enable)
{
	...
	DBG("enable=%d", enable);
	data = edp_read(aux->base + EDP_AUX_CTRL);

	if (enable) {
		data |= BIT(1); /* Reset */
		edp_write(aux->base + EDP_AUX_CTRL, data);
		/* Make sure full reset */
		wmb();
		usleep_range(500, 1000);

		data &= ~BIT(1); /* Clear Reset */
		data |= BIT(0); /* Enable */
		edp_write(aux->base + EDP_AUX_CTRL, data);
	} else {
		data &= ~BIT(0); /* Clear Enable */
		edp_write(aux->base + EDP_AUX_CTRL, data);
	}
}
