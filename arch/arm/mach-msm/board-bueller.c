...
#define MSM_ION_MM_SIZE		0x7300000
...
static struct platform_device apq8064_device_bt_wifi_reset_enable __devinitdata = {
	.name	= GPIO_REGULATOR_DEV_NAME,
	.id	= ATH_CHIP_PWD_L,
	.dev	= {
		.platform_data =
...
static struct platform_device bueller_apq8064_device_rpm_regulator __devinitdata = {
	.name	= "rpm-regulator",
	.id	= 0,
	.dev	= {
		.platform_data = &bueller_apq8064_rpm_regulator_pdata,
	},
};
...
static struct platform_device *bueller_early_common_devices[] __initdata = {
...
static struct platform_device *bueller_common_devices[] __initdata = {
...
static struct platform_device *bueller_apq_devices[] __initdata = {
	&mpq8064_device_uart_gsbi4,
	&msm_device_sps_apq8064,
	&mpq8064_device_qup_i2c_gsbi4,
...
static struct msm_spi_platform_data bueller_qup_spi_gsbi7_pdata = {
...
static struct dit4192_platform_data bueller_dit4192_pdata = {
	.gpio_buf_flip_int	= 29,
	.gpio_u_bits		= 30,
	.gpio_bls		= 31,
	.gpio_reset		= 35,
};

static struct spi_board_info bueller_spi_board_info[] __initdata = {
	{
		.modalias		= "dit4192-spdif",
		.max_speed_hz		= 1100000,
		.bus_num		= 2,
		.chip_select		= 0,
		.mode			= SPI_MODE_0,
		.platform_data		= &bueller_dit4192_pdata,
	},
...
static struct msm_i2c_platform_data mpq8064_i2c_qup_gsbi4_pdata = {
	.clk_freq = 100000,
...
static void __init mpq8064_i2c_init(void)
{
	...
	if (!gsbi_mem)
		...
		mpq8064_i2c_qup_gsbi4_pdata.use_gsbi_shared_mode = 1;
		mpq8064_device_qup_i2c_gsbi4.dev.platform_data =
				&mpq8064_i2c_qup_gsbi4_pdata;
	}
}
...
#define MPQ8064_I2C_GSBI4_BUS_ID	4
#define MPQ8064_I2C_GSBI5_BUS_ID	5
#define MPQ8064_I2C_GSBI1_BUS_ID	0

static struct i2c_board_info __initdata gsbi4_tmp105_info[] = {
	{
		I2C_BOARD_INFO("tmp105", 0x48),
	},
};

static struct i2c_board_info __initdata gsbi5_tmp105_info[] = {
	{
		I2C_BOARD_INFO("tmp105", 0x49),
	},
};

static struct i2c_registry bueller_i2c_devices[] __initdata = {
	{
		I2C_APQ_BUELLER,
		APQ_8064_GSBI4_QUP_I2C_BUS_ID,
		gsbi4_tmp105_info,
		ARRAY_SIZE(gsbi4_tmp105_info)
	},
	{
		I2C_APQ_BUELLER,
		APQ_8064_GSBI5_QUP_I2C_BUS_ID,
		gsbi5_tmp105_info,
		ARRAY_SIZE(gsbi5_tmp105_info)
	},
};
...
static void __init register_i2c_devices(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(bueller_i2c_devices); ++i) {
		if (bueller_i2c_devices[i].machs & mach_mask)
			i2c_register_board_info(
				bueller_i2c_devices[i].bus,
				bueller_i2c_devices[i].info,
				bueller_i2c_devices[i].len);
	}
}
...
static void enable_spdif_buffer(void)
{
	int rc;
	int spdif_buffer_enable = SPDIF_BUFFER_ENABLE_GPIO;

	rc = gpio_request(spdif_buffer_enable, "spdif_buffer_enable");
	if (rc) {
		pr_err("request for spdif_buffer_enable failed, rc=%d\n", rc);
		return;
	}

        rc = gpio_direction_output(spdif_buffer_enable, 1);
	if (rc) {
		pr_err("set dir for spdif_buffer_enable failed, rc=%d\n", rc);
		return;
	}
	pr_info("%s: done\n", __func__);
}
...
static void __init apq8064_common_init(void)
{
	...
	platform_device_register(&bueller_apq8064_device_rpm_regulator);

	platform_device_register(&apq8064_pm8921_device_rpm_regulator);
	if (msm_xo_init())
		pr_err("Failed to initialize XO votes\n");

	msm_clock_init(&bueller_apq8064_clock_init_data);
	...
	platform_add_devices(bueller_early_common_devices,
			ARRAY_SIZE(bueller_early_common_devices));

	printk(KERN_ERR "adding pm8921_mpq_hrd_common_devices\n");
	platform_add_devices(pm8921_mpq_hrd_common_devices,
		ARRAY_SIZE(pm8921_mpq_hrd_common_devices));

	platform_add_devices(bueller_common_devices, ARRAY_SIZE(bueller_common_devices));
...
static void __init apq8064_cdp_init(void)
{
	...
	enable_spdif_buffer();
	apq8064_device_qup_spi_gsbi7.dev.platform_data = &bueller_qup_spi_gsbi7_pdata;


	platform_add_devices(bueller_apq_devices,
		ARRAY_SIZE(bueller_apq_devices));

	spi_register_board_info(bueller_spi_board_info,
					ARRAY_SIZE(bueller_spi_board_info));
	...
	/* Hold a wakelock to prevent suspend - suspend is not supported in bueller */
	wake_lock_init(&bueller_wake_lock, WAKE_LOCK_SUSPEND, "prevent_bueller_suspend");
	wake_lock(&bueller_wake_lock);

}

MACHINE_START(APQ8064_BUELLER, "LAB126 BUELLER")
