...
long systrapz(unsigned int ctrl, unsigned int extra1, unsigned int extra2, unsigned int extra3, unsigned int extra4, trapz_info_t __user *ti);
...
long sys_trapz(unsigned int ctrl, unsigned int extra1, unsigned int extra2, unsigned int extra3, unsigned int extra4, struct trapz_info __user *ti);
#else /* __KERNEL */
#  define __NR_trapz 98
inline int systrapz(unsigned int ctrl, unsigned int extra1, unsigned int extra2, unsigned int extra3, unsigned int extra4, trapz_info_t *ti)
{
...
#define TRAPZ_TRACE_ID_IN(x) ((x & TRAPZ_TRACE_ID_MASK) << TRAPZ_TRACE_ID_OFFSET)

/* Log Macro definitions
 */
/* A simplified macro which uses the component category defined in trapz_generated.h */
#define TRAPZ_ILOG(level, flags, component, trace_id, extra1, extra2, extra3, extra4) \
  trapz_ilog(level, flags, component##__CAT, component##__ID, trace_id, extra1, extra2, extra3, extra4)
 ...
 */
#define TRAPZ_LOG(level, flags, component, trace, extra1, extra2, extra3, extra4) \
  TRAPZ_ILOG(level, flags, component, component##___##trace, extra1, extra2, extra3, extra4)
 ...
 */
#define TRAPZ_LOG_PRINTF(level, flags, component, trace, format, extra1, extra2, extra3, extra4) \
  TRAPZ_ILOG(level, flags, component, component##___##trace, extra1, extra2, extra3, extra4)
...
#ifdef __cplusplus
/* TRAPZ_LOG_SCOPE is a macro for automatically logging scope in C++.
 * It defines and instantiates a class which calls TRAPZ_LOG_BEGIN
 * in its constructor and TRAPZ_LOG_END in its destructor. When it goes out
 * of scope the destructor is called. This means for example that you
 * don't need a TRAPZ_LOG_END for every possible return from a function.
 */
#define TRAPZ_LOG_SCOPE(level, flags, component, trace) \
    class trace##_Class { \
    public: \
        trace##_Class() { \
            TRAPZ_LOG_BEGIN(level, flags, component, trace); \
        } \
        ~trace##_Class() { \
            TRAPZ_LOG_END(level, flags, component, trace); \
        } \
    }; \
    trace##_Class trace##_Instance
#endif /* __cplusplus */
...
#define TRAPZ_LOG_ENTER(level, flags, component, fn_ptr) \
  TRAPZ_ILOG(level, flags, component, component##___##fn_ptr, (int)fn_ptr, 1, 0, 0)
#define TRAPZ_LOG_EXIT(level, flags, component, fn_ptr) \
  TRAPZ_ILOG(level, flags, component, component##___##fn_ptr, (int)fn_ptr, 0, 0, 0)
...
#define trapz_ilog(level, flags, cat_id, comp_id, trace_id, extra1, extra2, extra3, extra4)  \
  trapz_ilog_info(level, flags, cat_id, comp_id, trace_id, extra1, extra2, extra3, extra4, NULL)

#define trapz_ilog_info(level, flags, cat_id, comp_id, trace_id, extra1, extra2, extra3, extra4, trapzinfo)  \
  systrapz(                             \
